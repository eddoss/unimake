...
from umk.project import Registerer as ProjectRegisterer
...
class DotUnimake:
    ...
    def _load_project(self, require: Require) -> states.State:
        ...
        try:
            ...
            return states.InternalError(description=str(e))
        ...
        for _, value in module.__dict__.items():
            if issubclass(type(value), ProjectRegisterer):
                self._instances.project = copy.deepcopy(value.instance)
                break

        if self._instances.project is None:
            return states.ProjectCreatorNotExists()
        elif not issubclass(type(self._instances.project), Project):
