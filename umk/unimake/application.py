...
def action(ctx: asyncclick.Context, remote: str, r: bool, c: tuple[str], p: tuple[str], f: bool, names: tuple[str]):
...
@application.group(cls=utils.ConfigableCommand, help="Run project target")
@asyncclick.option("--remote", default=None, help="Execute command in specific remote environment")
@asyncclick.option("-R", is_flag=True, default=False, help="Execute command in default remote environment. This flag has higher priority than --remote")
@asyncclick.argument('names', required=True, nargs=-1)
@asyncclick.pass_context
def target(ctx: asyncclick.Context, remote: str, r: bool, c: tuple[str], p: tuple[str], f: bool, names: tuple[str]):
    locally = not bool(remote or r)
    ...
    for name in valid:
        runtime.container.targets.run(name)
